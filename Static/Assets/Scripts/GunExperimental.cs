...
public class GunExperimental : MonoBehaviour {
	...
	void Update ()
	{
        ...
        currentPower = MyMath.Map(Mathf.Abs(gameManager.currentSine - sinePosition), 0f, nullDistance, 1f, 0f);
    ...
    {
        return curve.Evaluate(MyMath.Map(gameManager.currentSine, -1f, 1f, curve.keys[0].time, curve[curve.keys.Length - 1].time));
    }
