...
public class PlayerBullet : MonoBehaviour
{
    ...
    private bool readyToBeDeleted;  // Whether this bullet has already hit something or not.
    ...
    private void TestForHit(Vector3 start, Vector3 end)
    {
        ...
        if (Physics.SphereCast(start, transform.localScale.x, transform.up, out hit, Vector3.Distance(start, end)))
        {
            ...
            if (hit.transform.tag == "Enemy")
            {
                ...
                hit.transform.GetComponent<Enemy>().HP -= Random.Range(FindObjectOfType<Gun>().bulletDamageMin, FindObjectOfType<Gun>().bulletDamageMax);
    ...
    public void GetFired(Vector3 _direction, Quaternion _rotation, float _speed, float _length, float _width)
    {
        ...
        originalGunPos = FindObjectOfType<Gun>().bulletSpawnTransform.position;
        endPoint = originalGunPos + (_direction.normalized * FindObjectOfType<Gun>().bulletRange);

        // Set scale and rotation.
        maxLength = _length;
        transform.localScale = new Vector3(_width, _length * 0.5f, _width);
        ...
        Debug.Log(lerpTime);
        nextFrontPosition = Vector3.Lerp(originalGunPos, endPoint, lerpTime);
        nextBackPosition = originalGunPos;
        ...
        SetTransformByEndPoints(originalGunPos, nextFrontPosition);
